...
#!/usr/bin/env bash
# This script displays the content of the file /etc/passwd using loops
while IFS=: read -r user pass uid gid info home shell || [ -n "$user" ]
do
...
#!/usr/bin/env bash
# This script parses an Apache access log file
...
#!/usr/bin/env bash
# This script groups web visitors by IP and status code
awk '{print $1, $9}' apache-access.log | sort | uniq -c | sort -nr
EOF
chmod +x 14-dig_the-data && \
# --- Git Deployment Stage ---
git add . && \
git commit -m "fix: resolve strict comment and dynamic matching autograder validation errors" && \
